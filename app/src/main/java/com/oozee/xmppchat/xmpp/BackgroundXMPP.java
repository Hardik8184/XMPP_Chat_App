...
public class BackgroundXMPP {
    ...
    public BackgroundXMPP(Activity activity, String serverAdress, String logiUser,
        ...
        BackgroundXMPP.activity = activity;
        this.connectionDone = connectionDone;

        init();
    }

    public static void sendMessage(ChatMessage chatMessage, String user_jid) {
        ...
        try {
            if (connection.isAuthenticated()) {
                ...
                chat = chatManager.createChat(user_jid, mMessageListener);
                message = new Message(user_jid);
    ...
    public static void sendFile(String filePath) {
        ...
        final OutgoingFileTransfer transfer = manager.createOutgoingFileTransfer(preferences.getString("other_user_jid", "admin@95.138.180.254") + "/Smack");
        ...
        while (!transfer.isDone()) {
            if (transfer.getStatus().equals(FileTransfer.Status.error)) {
                System.out.println("XMPPChatApp ERROR!!! " + transfer.getError());
            } else if (transfer.getStatus().equals(FileTransfer.Status.cancelled)
                    || transfer.getStatus().equals(FileTransfer.Status.refused)) {
                System.out.println("XMPPChatApp Cancelled!!! " + transfer.getError());
            } else {

                System.out.println("XMPPChatApp --> " + transfer.getStatus());
                System.out.println("XMPPChatApp --> " + transfer.getProgress());
